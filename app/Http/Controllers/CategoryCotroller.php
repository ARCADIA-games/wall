...
class CategoryCotroller extends Controller
{
    function fetchAllCategory(Request $request){
        ...
        foreach ($categories as $cat) {
              ...
              $id =$cat->id;
              $news_count=  Wallpaper::Where('wallpaper_type',0)->where('category_id',$id)->count();
    ...
    function deleteCat($id){
           ...
           Wallpaper::where('category_id',$id)->where('wallpaper_type',0)->delete();
    ...
    function updateCat(Request $req){
        ...
        if($req->image == ""){
            Category::where('id', $req->id)->update(['title' => $req->title]);
        }else{
            $path = $req->file('image')->store('uploads');
            Category::where('id', $req->id)->update(['title' => $req->title,'image' => $path ]);
    ...
    function getCategories(){
        ...
        foreach ($row1 as $cat) {
           ...
           $cat_id = $cat->id;
           $row = Wallpaper::Where('wallpaper_type',0)->where('category_id',$cat_id)->count();
           $cat['count'] = $row;
           $data[] = $cat;
